...
int main(int argc, char *argv[])
{
	 ...
	 * test word: baseball
	 * initial(before parallel): .116142
	 *
	 * notes:
	 * same loops as t2_fastest
	 * interchange first and second loop with 2 and 4, insignificant change
	 * loop average: .0441464
	 */
	#pragma omp parallel for shared(wl_size, num_hf, wl, bv_size) schedule(guided, 4) private(j, hash)
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	misspelled = 0;
	#pragma omp parallel for shared(wl_size, num_hf, wl, bv_size) schedule(guided, 4) private(hash)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
