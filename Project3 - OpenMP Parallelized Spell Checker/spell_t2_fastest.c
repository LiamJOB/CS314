...
int main(int argc, char *argv[])
{
	...
	/* each test run 5 times, then averaged
	 * test word: baseball
	 * initial(before parallel): .1453338
	 *
	 * notes:
	 * added second parallelization to spell checking
	 * #pragma omp parallel for shared(wl_size, num_hf, wl, bv_size) schedule(guided) private(hash)
	 *	average: .074537 (best so far)
	 */
#pragma omp parallel for shared(wl_size, num_hf, wl, bv_size) schedule(guided, 2) private(i, j, hash)
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	#pragma omp parallel for shared(wl_size, num_hf, wl, bv_size) schedule(guided, 2) private(j, hash)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
